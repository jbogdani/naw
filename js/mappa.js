const mappa = () => {
    ...
    pois.forEach(p => {
        ...
        let fromToText = lang === 'it' ? `<p><strong>Attività Sapenza</strong><br> ${p.from} – ${p.to}</p>` : `<p><strong>Sapienza activities</strong><br> ${p.from} — ${p.tp}</p>`;
        ...
        const marker = L.circleMarker(p.coord, {
            ...
            .bindPopup(`<h5 class="text-center"><a href="./${p.url}" title="${p.name}">${p.name}</a></h5>
